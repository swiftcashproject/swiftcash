...
*Setup instructions for a gitian build of SwiftCash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SwiftCash
...
- [Building SwiftCash](#building-swiftcash)
...
After the VM has booted you can connect to it using SSH, and files can be copied from and to the VM using a SWIFTP utility.
...
Clone the git repositories for swiftcash and gitian and then checkout the swiftcash version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the swiftcash repository
...
To build SwiftCash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the swiftcash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/swiftcash/.git/
...
COMMIT=aff5fb8eg0019a019b73f03001091aaef1149b91
./bin/gbuild --commit swiftcash=${COMMIT} --url swiftcash=${URL} ../swiftcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit swiftcash=${COMMIT} --url swiftcash=${URL} ../swiftcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit swiftcash=${COMMIT} --url swiftcash=${URL} ../swiftcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/swiftcash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/swiftcash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/swiftcash-build.assert
```
...
[swiftcash/gitian.sigs](https://github.com/swiftcashproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
