...
/**
 * Fees smaller than this (in uswift) are considered zero fee (for transaction creation)
...
bool CWallet::GetSwiftnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetSwiftnodeVinAndKeys -- Could not locate any valid swiftnode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetSwiftnodeVinAndKeys -- Could not locate specified swiftnode vin\n");
    return false;
}
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fSwiftNode && vout[i].nValue == SWIFTNODE_COLLATERAL * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add swiftnode collaterals which are handled like locked coins
        else if (fSwiftNode && vout[i].nValue == SWIFTNODE_COLLATERAL * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
CAmount CWalletTx::GetLockedWatchOnlyCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add swiftnode collaterals which are handled likc locked coins
        else if (fSwiftNode && vout[i].nValue == SWIFTNODE_COLLATERAL * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_WATCH_ONLY);
        }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_NOT10000IFMN) {
                    found = !(fSwiftNode && pcoin->vout[i].nValue == SWIFTNODE_COLLATERAL * COIN);
                } if (nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == SWIFTNODE_COLLATERAL * COIN;
                } else {
                    found = true;
                }
                if (!found) continue;
...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        //check for min age/depth
        int64_t nTxTime = mapBlockIndex.at(out.tx->hashBlock)->GetBlockTime();
        if (chainActive.Height() > 1000 && (GetAdjustedTime() - nTxTime < nStakeMinAge && out.nDepth < (int)nStakeMinDepth))
            continue;
...
bool CWallet::MintableCoins()
{
    ...
    for (const COutput& out : vCoins) {
        //check for min age/depth and min value
        int64_t nTxTime = mapBlockIndex.at(out.tx->hashBlock)->GetBlockTime();
        if (out.tx->vout[out.i].nValue >= nStakeMinValue &&
            (GetAdjustedTime() - nTxTime >= nStakeMinAge || out.nDepth >= (int)nStakeMinDepth))
                return true;
    }
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX, bool isProposal)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    CAmount trxFee = isProposal ? PROPOSAL_FEE_TX : BUDGET_FEE_TX;
    vecSend.push_back(make_pair(scriptChange, trxFee));

    CCoinControl* coinControl = NULL;
    bool success = CreateTransaction(vecSend, false, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if (!success) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SWIFT.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-swift-address
                    CScript scriptChange;
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, fSubtractFeeFromAmount, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, coin_type, useIX, nFeePay);
}

// swiftcash: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount nFees, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for(PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            if (nTotalSize > 0 && nTotalSize / 2 > (int64_t)nStakeSplitThreshold * COIN)
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        //find swiftnode rewards that need to be combined
        ...
        if (!CreateTransaction(vecSend, false, wtx, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0))) {
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        bool sendMSonMNReward = fMultiSendSwiftnodeReward && outpoint.IsSwiftnodeReward(out.tx);
        ...
        CWalletTx wtxdummy;
        string strErr;
        CreateTransaction(vecSend, false, wtxdummy, keyChange, nFeeRet, strErr, &cControl, ALL_COINS, false, CAmount(0));
        ...
        // Create the transaction and commit it to the network
        if (!CreateTransaction(vecSend, false, wtx, keyChange, nFeeRet, strErr, &cControl, ALL_COINS, false, CAmount(0))) {
        ...
        if (!walletdb.WriteMSettings(fMultiSendStake, fMultiSendSwiftnodeReward, nLastMultiSendHeight))
        ...
        if ((stakeSent && mnSent) || (stakeSent && !fMultiSendSwiftnodeReward) || (mnSent && !fMultiSendStake))
            return true;
    }
