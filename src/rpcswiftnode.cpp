...
void SendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType coin_type = ALL_COINS)
{
    ...
    if (!pwalletMain->CreateTransaction(scriptPubKey, nValue, false, wtxNew, reservekey, nFeeRequired, strError, NULL, coin_type)) {
...
UniValue getroi(const UniValue& params, bool fHelp)
{
    ...
    if(swiftnodes == 0) {
       ...
       swiftnode = ((double)blockRewards*1/5/COIN)/(swiftnodes*SWIFTNODE_COLLATERAL);
    }

    staking = ((double)blockRewards*4/5/COIN)/((double)GetDifficulty()*10);
