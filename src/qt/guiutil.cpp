...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SWIFT address (e.g. %1)").arg("Sh7nUAg9fWxqhCL2KiqWarsF6VfPBPvJz4"));
...
{
    // return if URI is not valid or is no swiftcash: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::SWIFT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert swiftcash:// to swiftcash:
    //
    //    Cannot handle this later, because swiftcash:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SWIFT, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open swiftcash.conf with the associated application */
...
{
    boost::filesystem::path pathConfig = GetSwiftnodeConfigFile();

    /* Open swiftnode.conf with the associated application */
    if (boost::filesystem::exists(pathConfig))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathConfig)));
}
 ...
 * the "active-since" list in the swiftnode tab are sorted by the elapsed
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SwiftCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SwiftCash.lnk
...
{
    return GetAutostartDir() / "swiftcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a swiftcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SwiftCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the swiftcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add swiftcash app to startup item list
