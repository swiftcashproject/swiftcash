...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendSwiftnodeCheckBox->setChecked(pwalletMain->fMultiSendSwiftnodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake && pwalletMain->fMultiSendSwiftnodeReward)
            strStatus += tr("MultiSend Active for Stakes and Swiftnode Rewards") + "\n";
        else if (pwalletMain->fMultiSendStake)
            strStatus += tr("MultiSend Active for Stakes") + "\n";
        else if (pwalletMain->fMultiSendSwiftnodeReward)
            strStatus += tr("MultiSend Active for Swiftnode Rewards") + "\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendSwiftnodeCheckBox->isChecked())) {
        strRet = tr("Need to select to send on stake and/or swiftnode rewards");
        ...
        pwalletMain->fMultiSendSwiftnodeReward = ui->multiSendSwiftnodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendSwiftnodeReward, pwalletMain->nLastMultiSendHeight))
