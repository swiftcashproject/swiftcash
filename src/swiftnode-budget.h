...
#ifndef SWIFTNODE_BUDGET_H
#define SWIFTNODE_BUDGET_H
...
//
// CBudgetVote - Allow a swiftnode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a swiftnode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenSwiftnodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenSwiftnodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanSwiftnodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenSwiftnodeBudgetProposals.clear();
        mapSeenSwiftnodeBudgetVotes.clear();
    ...
    TrxValidationStatus IsTransactionValid(const CTransaction& txNew, int nBlockHeight, CAmount& nBudgetPaid);
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanSwiftnodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        READWRITE(mapSeenSwiftnodeBudgetProposals);
        READWRITE(mapSeenSwiftnodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanSwiftnodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (swiftnode only)
...
public:
    ...
    void AutoCheck();
    //total swift paid out by this budget
    CAmount GetTotalPayout();
    //vote on this finalized budget as a swiftnode
    void SubmitVote();
...
//
// Budget Proposal : Contains the swiftnode votes for each budget
//
...
public:
    ...
    */
    ...
    bool IsEstablished()
    {
        // Proposals must be at least 10 days old to make it into a budget
        if (Params().NetworkID() == CBaseChainParams::MAIN) return (nTime < GetTime() - (60 * 60 * 24 * 10));
