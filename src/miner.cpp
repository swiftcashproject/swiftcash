...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // swiftcash: if coinstake available add coinstake tx
...

    {
        ...
        if (!fProofOfStake) {
            //Swiftnode and general budget payments
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SwiftCashMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SwiftCashMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SwiftCashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("swiftcash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SwiftCashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
