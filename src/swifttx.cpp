...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL swiftnodes, open connect to top 1 swiftnode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 swiftnode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == "txlvote") // SwiftTX Lock Consensus Votes
    {
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            //Spam/Dos protection
            /*
                Swiftnodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinSwiftnode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinSwiftnode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinSwiftnode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinSwiftnode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - swiftnode is spamming transaction votes: %s %s\n",
                        ctx.vinSwiftnode.ToString().c_str(),
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_3_MAX_VALUE) * COIN) {
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which swiftnodes
        they'll use.
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        newLock.nExpiration = GetTime() + (60 * 60); //locks expire after 60 minutes (10 confirmations)
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetSwiftnodeRank(activeSwiftnode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swifttx", "SwiftTX::DoConsensusVote - Unknown Swiftnode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swifttx", "SwiftTX::DoConsensusVote - Swiftnode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinSwiftnode = activeSwiftnode.vin;
...
{
    int n = mnodeman.GetSwiftnodeRank(ctx.vinSwiftnode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CSwiftnode* pmn = mnodeman.Find(ctx.vinSwiftnode);
    if (pmn != NULL)
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Swiftnode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Unknown Swiftnode\n");
        mnodeman.AskForMN(pnode, ctx.vinSwiftnode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swifttx", "SwiftTX::ProcessConsensusVote - Swiftnode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftTX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced swiftnode
        mnodeman.AskForMN(pnode, ctx.vinSwiftnode);
        return false;
    }
...
{
    return vinSwiftnode.prevout.hash + vinSwiftnode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftTX::CConsensusVote::SignatureValid() - Unknown Swiftnode\n");
        return false;
    }

    if (!swiftnodeSigner.VerifyMessage(pmn->pubKeySwiftnode, vchSwiftNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strSwiftNodePrivKey.c_str());

    if (!swiftnodeSigner.SetKey(strSwiftNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid swiftnodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if (!swiftnodeSigner.SignMessage(strMessage, errorMessage, vchSwiftNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!swiftnodeSigner.VerifyMessage(pubkey2, vchSwiftNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetSwiftnodeRank(vote.vinSwiftnode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Swiftnode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Swiftnode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
