...
#if defined(NDEBUG)
#error "SwiftCash cannot be compiled without assertions."
#endif
...
int64_t GetSwiftnodePayment(int nHeight, int64_t blockValue, int nSwiftnodeCount)
{
    int64_t ret = 0;

    // No rewards till swiftnode activation.
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 100 * 10 * 60); // ~100 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 50 blocks (+/- ~8 hours if no one mines it)
    // of our head, drop it
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 50)
...
{
    RenameThread("swiftcash-scriptch");
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // swiftcash: compute chain trust score
        pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

        // swiftcash: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

        // swiftcash: record proof-of-stake hash value
        if (pindexNew->IsProofOfStake()) {
        ...
        // swiftcash: compute stake modifier
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, CAmount& nBudgetPaid)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            if (fDebug)
                LogPrintf("CheckBlock(): Swiftnode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
...
bool CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned int nRequired)
{
    ...
    if (chainActive.Height() > (int)nToCheck && minVersion <=5) return true;
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    // swiftcash: check proof-of-stake
    ...
    if (!fLiteMode) {
        if (swiftnodeSync.RequestedSwiftnodeAssets > SWIFTNODE_SYNC_LIST) {
            swiftnodePayments.ProcessBlock(GetHeight() + 10);
            budget.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_SWIFTNODE_WINNER:
        if (swiftnodePayments.mapSwiftnodePayeeVotes.count(inv.hash)) {
            swiftnodeSync.AddedSwiftnodeWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_VOTE:
        if (budget.mapSeenSwiftnodeBudgetVotes.count(inv.hash)) {
            swiftnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_PROPOSAL:
        if (budget.mapSeenSwiftnodeBudgetProposals.count(inv.hash)) {
            swiftnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    ...
    case MSG_BUDGET_FINALIZED:
        ...
        return false;
    case MSG_SWIFTNODE_ANNOUNCE:
        if (mnodeman.mapSeenSwiftnodeBroadcast.count(inv.hash)) {
            swiftnodeSync.AddedSwiftnodeList(inv.hash);
            return true;
        }
        return false;
    case MSG_SWIFTNODE_PING:
        return mnodeman.mapSeenSwiftnodePing.count(inv.hash);
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_SWIFTNODE_WINNER) {
                    if (swiftnodePayments.mapSwiftnodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << swiftnodePayments.mapSwiftnodePayeeVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenSwiftnodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenSwiftnodeBudgetVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenSwiftnodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenSwiftnodeBudgetProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_SWIFTNODE_ANNOUNCE) {
                    if (mnodeman.mapSeenSwiftnodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSwiftnodeBroadcast[inv.hash];
                        pfrom->PushMessage("mnb", ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_SWIFTNODE_PING) {
                    if (mnodeman.mapSeenSwiftnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSwiftnodePing[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // getaddr message mitigates the attack.
        ...
        swiftnodePayments.ProcessMessageSwiftnodePayments(pfrom, strCommand, vRecv);
        ProcessMessageSwiftTX(pfrom, strCommand, vRecv);
        ProcessSpork(pfrom, strCommand, vRecv);
        swiftnodeSync.ProcessMessage(pfrom, strCommand, vRecv);
    }
