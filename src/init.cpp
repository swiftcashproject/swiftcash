...
void PrepareShutdown()
{
    ...
    RenameThread("swiftcash-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "swiftcash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "swiftcashd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8544, 28544));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SWIFT/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SWIFT/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, swiftcash, (swifttx, swiftnode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SWIFT/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SwiftCash specific functionality (Swiftnodes, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("Swiftnode options:"));
    strUsage += HelpMessageOpt("-swiftnode=<n>", strprintf(_("Enable the client to act as a swiftnode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify swiftnode configuration file (default: %s)"), "swiftnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock swiftnodes from swiftnode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-swiftnodeprivkey=<n>", _("Set the swiftnode private key"));
    strUsage += HelpMessageOpt("-swiftnodeaddr=<n>", strprintf(_("Set external address:port to get to this swiftnode (example: %s)"), "128.127.106.235:8544"));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8543, 28543));
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i Bitcoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i Dash Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i PIVX Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i SwiftCash Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_("This is an experimental software.")) + "\n" +
           "\n" +
...
{
    RenameThread("swiftcash-loadblk");
...
/** Sanity checks
 *  Ensure that SwiftCash is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SwiftCash Core is shutting down."));
...
#endif
    // Make sure only a single SwiftCash process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SwiftCash Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SwiftCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // SwiftCash: load previous sessions sporks if we have them.
                ...
                if (!CVerifyDB().VerifyDB(pcoinsdbview, GetArg("-checklevel", 3), GetArg("-checkblocks", 100))) {
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SwiftCash Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SwiftCash Core to complete") << "\n";
    ...
    // ********************************************************* Step 10: setup Swiftnode

    uiInterface.InitMessage(_("Loading swiftnode cache..."));

    CSwiftnodeDB mndb;
    CSwiftnodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CSwiftnodeDB::FileError)
        LogPrintf("Missing swiftnode cache file - mncache.dat, will try to recreate\n");
    ...
    uiInterface.InitMessage(_("Loading swiftnode payment cache..."));

    CSwiftnodePaymentDB mnpayments;
    CSwiftnodePaymentDB::ReadResult readResult3 = mnpayments.Read(swiftnodePayments);

    if (readResult3 == CSwiftnodePaymentDB::FileError)
        LogPrintf("Missing swiftnode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CSwiftnodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CSwiftnodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    fSwiftNode = GetBoolArg("-swiftnode", false);

    if ((fSwiftNode || swiftnodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Swiftnode support requires turning on transaction indexing."
                         "Please add txindex=1 to your configuration and start with -reindex");
    }

    if (fSwiftNode) {
        LogPrintf("IS MASTER NODE\n");
        strSwiftNodeAddr = GetArg("-swiftnodeaddr", "");

        LogPrintf(" addr %s\n", strSwiftNodeAddr.c_str());

        if (!strSwiftNodeAddr.empty()) {
            CService addrTest = CService(strSwiftNodeAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -swiftnodeaddr address: " + strSwiftNodeAddr);
            }
        }

        strSwiftNodePrivKey = GetArg("-swiftnodeprivkey", "");
        if (!strSwiftNodePrivKey.empty()) {
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if (!swiftnodeSigner.SetKey(strSwiftNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid swiftnodeprivkey. Please see documenation."));
            }

            activeSwiftnode.pubKeySwiftnode = pubkey;

        } else {
            return InitError(_("You must specify a swiftnodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    //get the mode of budget voting for this swiftnode
    strBudgetMode = GetArg("-budgetvotemode", "auto");

    if (GetBoolArg("-mnconflock", true) && pwalletMain) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Swiftnodes:\n");
        uint256 mnTxHash;
    ...
    //lite mode disables all Swiftnode related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fSwiftNode && fLiteMode) {
        return InitError("You can not start a swiftnode in litemode");
    }
    ...
    swiftnodeSigner.InitCollateralAddress();

    threadGroup.create_thread(boost::bind(&ThreadSwiftnodePool));
