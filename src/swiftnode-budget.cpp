...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
	...
	CAmount trxRequiredFee = fBudgetFinalization ? BUDGET_FEE_TX : PROPOSAL_FEE_TX;
        if (o.scriptPubKey == findScript && o.nValue >= trxRequiredFee) foundOpReturn = true;
    }
    if (!foundOpReturn) {
...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CBudgetVote>::iterator it1 = mapOrphanSwiftnodeBudgetVotes.begin();
    while (it1 != mapOrphanSwiftnodeBudgetVotes.end()) {
        if (budget.UpdateProposal(((*it1).second), NULL, strError)) {
            ...
            mapOrphanSwiftnodeBudgetVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        CWalletTx wtx;
        if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, tempBudget.GetHash(), false, false)) {
...
CBudgetDB::CBudgetDB()
{
    ...
    strMagicMessage = "SwiftnodeBudget";
}
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 5%% of Swiftnodes: %lli. Number of finalized budgets: %lli\n", 
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());

    // If budget doesn't have 5% of the network votes, then we should pay a swiftnode instead
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint("mnbudget","CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 5%% of Swiftnodes: %lli mapFinalizedBudgets.size(): %ld\n",
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 5% of the network votes, then we should pay a swiftnode instead
    */
    ...
    // If not enough swiftnodes autovoted for any of the finalized budgets pay a swiftnode instead
    if(!fThreshold) {
...
std::vector<CBudgetProposal*> CBudgetManager::GetBudget()
{
    ...
    while (it1 != mapProposals.end()) {
        ...
        if (nHighestCount < pBudgetProposal->GetVoteCount() &&
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight < 10000)
        nSubsidy = 15 * COIN; // fair launch - give appx 1 week to users to set up their wallets and nodes
    else
...
void CBudgetManager::NewBlock()
{
    ...
    if (swiftnodeSync.RequestedSwiftnodeAssets <= SWIFTNODE_SYNC_BUDGET) return;
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnvs") { //Swiftnode vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        Sync(pfrom, nProp);
        LogPrint("mnbudget", "mnvs - Sent Swiftnode votes to peer %i\n", pfrom->GetId());
    }

    if (strCommand == "mprop") { //Swiftnode Proposal
        CBudgetProposalBroadcast budgetProposalBroadcast;
        vRecv >> budgetProposalBroadcast;

        if (mapSeenSwiftnodeBudgetProposals.count(budgetProposalBroadcast.GetHash())) {
            swiftnodeSync.AddedBudgetItem(budgetProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenSwiftnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
    ...
    if (strCommand == "mvote") { //Swiftnode Vote
        CBudgetVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if (mapSeenSwiftnodeBudgetVotes.count(vote.GetHash())) {
            swiftnodeSync.AddedBudgetItem(vote.GetHash());
            return;
        }

        CSwiftnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget","mvote - unknown swiftnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }


        mapSeenSwiftnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
    ...
    if (strCommand == "fbs") { //Finalized Budget Suggestion
        ...
        if (mapSeenFinalizedBudgets.count(finalizedBudgetBroadcast.GetHash())) {
            swiftnodeSync.AddedBudgetItem(finalizedBudgetBroadcast.GetHash());
            return;
        }
    ...
    if (strCommand == "fbvote") { //Finalized Budget Vote
        ...
        CSwiftnode* pmn = mnodeman.Find(vote.vin);
        if (pmn == NULL) {
            LogPrint("mnbudget", "fbvote - unknown swiftnode - vin: %s\n", vote.vin.prevout.hash.ToString());
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }
...
void CBudgetManager::ResetSync()
{
    ...
    std::map<uint256, CBudgetProposalBroadcast>::iterator it1 = mapSeenSwiftnodeBudgetProposals.begin();
    while (it1 != mapSeenSwiftnodeBudgetProposals.end()) {
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", SWIFTNODE_SYNC_BUDGET_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", SWIFTNODE_SYNC_BUDGET_FIN, nInvCount);
    LogPrint("mnbudget", "CBudgetManager::Sync - sent %d items\n", nInvCount);
}
...
// If swiftnode voted for a proposal, but is now invalid -- remove the vote
...
bool CBudgetVote::Sign(CKey& keySwiftnode, CPubKey& pubKeySwiftnode)
{
    // Choose coins to use
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        if (fDebug){
            LogPrint("mnbudget","CBudgetVote::SignatureValid() - Unknown Swiftnode - %s\n", vin.prevout.hash.ToString());
        }
        return false;
    }
...
//evaluate if we should vote for this. Swiftnode only
void CFinalizedBudget::AutoCheck()
{
    ...
    if (!fSwiftNode || fAutoChecked) {
        LogPrint("mnbudget","CFinalizedBudget::AutoCheck fSwiftNode=%d fAutoChecked=%d\n", fSwiftNode, fAutoChecked);
        return;
    }
...
bool CFinalizedBudget::IsValid(std::string& strError, bool fCheckCollateral)
{
    ...
    // Can only pay out 60% of the possible coins (min value of coins)
    if (GetTotalPayout() > budget.GetTotalBudget(nBlockStart)) {
        strError = "Budget " + strBudgetName + " (" + strProposals + ") Invalid Payout (more than max) - can only pay out 60% of the possible coins";
        return false;
    }
...
bool CFinalizedBudget::IsPaidAlready(uint256 nProposalHash, int nBlockHeight)
{
    ...
    // This budget was paid already -> reject transaction so it gets paid to a swiftnode instead
    return true;
}
...
TrxValidationStatus CFinalizedBudget::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    BOOST_FOREACH (CTxOut out, txNew.vout) {
        ...
        if (vecBudgetPayments[nCurrentBudgetPayment].payee == out.scriptPubKey && vecBudgetPayments[nCurrentBudgetPayment].nAmount == out.nValue) {
            // Check if this proposal was paid already. If so, pay a swiftnode instead
            paid = IsPaidAlready(vecBudgetPayments[nCurrentBudgetPayment].nProposalHash, nBlockHeight);
            if(paid) {
                LogPrint("mnbudget","CFinalizedBudget::IsTransactionValid - Double Budget Payment of %d for proposal %d detected. Paying a swiftnode instead.\n",
...
void CFinalizedBudget::SubmitVote()
{
    ...
    CFinalizedBudgetVote vote(activeSwiftnode.vin, GetHash());
...
bool CFinalizedBudgetVote::Sign(CKey& keySwiftnode, CPubKey& pubKeySwiftnode)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("mnbudget","CFinalizedBudgetVote::SignatureValid() - Unknown Swiftnode %s\n", strMessage);
        return false;
    }
...
std::string CBudgetManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() << ", Budgets: " << (int)mapFinalizedBudgets.size() << ", Seen Budgets: " << (int)mapSeenSwiftnodeBudgetProposals.size() << ", Seen Budget Votes: " << (int)mapSeenSwiftnodeBudgetVotes.size() << ", Seen Final Budgets: " << (int)mapSeenFinalizedBudgets.size() << ", Seen Final Budget Votes: " << (int)mapSeenFinalizedBudgetVotes.size();
